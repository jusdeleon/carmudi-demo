...
## Requirements
 ...
 - You are **NOT ALLOWED** to use any `web frameworks` (e.g. Laravel, Symfony). Non-`web framework` libraries are ok to use though (e.g. Eloquent, Symfony Components), as long as it is in PHP and can be installed with composer. We will be particularly interested in the way that measurement unit problem is solved, so be careful if you are using a library for this specific problem, because the library code will be considered.
